...
**Session History**: A local JSONL file (`~/.diskwise/session-history.jsonl`) that accumulates session summaries over time. The agent reads this at the start of each session to inform Claude about patterns from previous runs.
