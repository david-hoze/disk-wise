...
callClaudeCode sysPrompt userPrompt = do
  let fullPrompt = userPrompt
      args = ["--print", "--system-prompt", T.unpack sysPrompt, T.unpack fullPrompt]
...
buildSystemPrompt = T.unlines
  ...
  , "      \"summary\": \"Commit message describing the change\""
  , "    }"
  , "  ]"
  , "}"
  , ""
  ...
  , "- Propose wiki_contributions when you discover a NEW tool/pattern not already in the wiki"
  , "- Use CreatePage for new tools, AmendPage to add info to existing pages"
  , "- Wiki pages should be generic (useful to anyone), not user-specific"
  , "- Replace absolute home paths with ~ in wiki content"
  , "- Do NOT propose wiki contributions for patterns already well-covered by existing pages"
  ]
...
parseAdvice text =
  -- Try to find JSON in the response (Claude might wrap it in markdown code blocks)
